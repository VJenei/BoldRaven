...
(function () {
  ...
  var POLICIES = DB.policies;                 // fixed display order, 8 entries
  ...
  function navigate(code, updateHash) {
  ...
  function codeFromHash() {
    ...
    return (/^\d{6}$/.test(raw) && CODES[raw]) ? raw : null;
  }

  window.addEventListener('hashchange', function () {
    navigate(codeFromHash(), false);
  });
